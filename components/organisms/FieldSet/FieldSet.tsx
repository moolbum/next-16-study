...
export const FieldSet = ({ items, ...props }: FieldSetProps) => {
	...
	const renderDescription = (item: FieldSetItems, description?: string | React.ReactNode) => {
		if (isString(description)) {
			return <FieldDescription {...item.descriptionProps}>{description}</FieldDescription>;
		}
		return description;
	};
