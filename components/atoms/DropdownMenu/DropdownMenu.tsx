...
export const DropdownMenu = ({
	...
	// 트리거 렌더링
	const triggerElement = trigger || <Button variant="outline">{triggerLabel || 'Open'}</Button>;
	...
	return (
		<DropdownMenuComponent modal={modal}>
			...
			<DropdownMenuContent
				...
				style={typeof contentWidth === 'number' ? { width: `${contentWidth}px` } : undefined}
				align={align}
			>
					...
					<React.Fragment key={groupIndex}>
						...
						<DropdownMenuGroup>
							{group.items.map((item, itemIndex) => {
								const { label, value, shortcut, icon, separator, onClick, ...itemProps } = item;
								return (
									<React.Fragment key={value || itemIndex}>
										<DropdownMenuItem {...itemProps} onSelect={onClick || itemProps.onSelect}>
											{icon}
											<span>{label}</span>
						...
						{group.separator && groupIndex < groupsToRender.length - 1 && <DropdownMenuSeparator />}
					</React.Fragment>
				))}
			</DropdownMenuContent>
		</DropdownMenuComponent>
